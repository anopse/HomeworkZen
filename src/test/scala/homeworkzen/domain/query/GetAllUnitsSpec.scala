...
class GetAllUnitsSpec extends FlatSpec with Matchers with MockFactory {
  ...
  "GetAllUnits" should "return empty collection with an empty journal" in {
    ...
    val result = Await.result(GetAllUnits(user.userId), 500.milliseconds)
    result shouldBe Seq.empty
  }

  it should "return all matching units with a filled journal" in {
    implicit val journal = mock[JournalReader]
    ...
    val result = Await.result(GetAllUnits(user.userId), 500.milliseconds)
    result shouldBe user.units.map(_.unitInfo)
  }
}
